...
บทนี้แบ่งตัวอย่างเป็นเซลล์สั้น ๆ สำหรับ Jupyter Notebook หรือ Google Colab:
...
ค่าจาก Interpolation เป็นค่าประมาณ ไม่ใช่ค่าที่วัดจริง จึงไม่ควรนำเสนอว่ามีความแม่นยำเท่ากับค่าที่ตำแหน่ง Sensor Node ยิ่งเซนเซอร์อยู่ห่างกันมากหรือมีสิ่งกีดขวางทางกายภาพ ความไม่แน่นอนยิ่งสูง
...
ในตัวอย่างแรก ค่า Z มาจาก `make_temperature_field()` ซึ่งเป็นสนามอุณหภูมิจำลองที่ทราบค่าทุกตำแหน่ง กราฟนี้จึงใช้สอนวิธีอ่าน Contour ก่อน ส่วนแผนที่ที่สร้างจาก Sensor Node เพียงไม่กี่จุดจะทำในหัวข้อ Interpolation ภายหลัง
...
ก่อนสร้างกราฟ ให้เขียนฟังก์ชันช่วยสำหรับตั้งค่าแกน ฟังก์ชันนี้ยังไม่สร้าง Figure จึงแยกเป็นอีกเซลล์ได้
...
ตัวอย่างนี้สร้างกราฟใน Figure ใหม่ จึงรวมคำสั่งทั้งหมดไว้ในอีกหนึ่งเซลล์
...
- Colorbar ต้องมีชื่อและหน่วยเพื่อให้ตีความสีได้ถูกต้อง
...
กราฟตัวอย่างนี้รวมภาพรวมของอุณหภูมิ ระดับสำคัญ ตำแหน่งอุปกรณ์ และค่าที่วัดได้ไว้ด้วยกัน
...
`coolwarm` เหมาะเมื่อต้องการเน้นบริเวณเย็นและร้อนรอบค่าปกติ แต่ถ้าต้องการสื่อเพียง “ต่ำไปสูง” ควรทดลอง `viridis` หรือ `inferno`
...
กราฟก่อนหน้านี้ใช้สนามอุณหภูมิจำลองซึ่งทราบค่าทุกตำแหน่ง แต่ข้อมูลจริงมีเพียง X, Y และค่าจาก Sensor Node แต่ละตัว จึงต้องประมาณค่าบนกริดก่อนส่งให้ `contourf()` หรือ `imshow()`
...
ตอนนี้พื้นที่สีคำนวณจากค่าที่จำลองว่า Sensor Node วัดได้ ไม่ได้ใช้สนามอุณหภูมิเต็มโดยตรง จุดที่อยู่นอกขอบเขตการติดตั้งเซนเซอร์จะไม่มีสี เพราะ Linear Interpolation ไม่ควรประมาณค่าออกไปไกลจากจุดวัด

### เซลล์ที่ 17 — ประเมินความคลาดเคลื่อนของ Interpolation

ข้อมูลจำลองมีค่าอุณหภูมิจริงทุกตำแหน่ง จึงใช้เปรียบเทียบกับค่าประมาณได้:
...
ในงานจริงไม่มีค่าจริงครบทุกตำแหน่ง วิธีประเมินที่เหมาะกว่าคือซ่อนข้อมูลเซนเซอร์บางตัวไว้ ทำ Interpolation จากเซนเซอร์ที่เหลือ แล้วเปรียบเทียบค่าประมาณกับค่าที่ซ่อนไว้ แนวทางนี้คล้าย cross-validation
...
ตัวอย่างในบทนี้มีทั้งสนามอุณหภูมิจำลองที่ทราบค่าทุกตำแหน่ง และแผนที่ที่ประมาณจาก Sensor Node เพียงไม่กี่จุด เมื่อเปลี่ยนไปใช้ข้อมูลจริงต้องแยกค่าที่วัดได้ออกจากค่าที่ประมาณอย่างชัดเจน
