...
กราฟหนึ่งรูปตอบคำถามได้หนึ่งมุม แต่การตัดสินใจในโรงงานมักต้องดูหลายตัวแปรพร้อมกัน เช่น อัตราการผลิตลดลงหรือไม่ อุณหภูมิ Motor สูงขึ้นพร้อมกับแรงสั่นสะเทือนหรือไม่ และ Defect Rate เปลี่ยนไปในช่วงเวลาเดียวกันหรือไม่
...
- สร้าง Layout ที่กราฟครอบคลุมหลายช่องด้วย `GridSpec`
...
| Axes | พื้นที่กราฟหนึ่งช่อง เช่น กราฟอุณหภูมิหนึ่งช่อง |
| Axis | ระบบแกน X หรือแกน Y ภายใน Axes ไม่ใช่คำเดียวกับ Axes |
| Subplot | Axes ที่ Matplotlib จัดวางเป็นส่วนหนึ่งของ Layout ภายใน Figure |
...
เมื่อมีหลาย Axes ชื่อ `axes` ช่วยบอกว่าตัวแปรนี้เป็น Array ไม่ใช่ Axes เดี่ยว
...
`axes.flat` เรียง Axes จาก Array 2×2 ให้เป็นลำดับต่อเนื่อง จึงจับคู่กับรายการ `series` ผ่าน `zip()` ได้โดยไม่ต้องเขียนคำสั่ง Plot ซ้ำสี่ครั้ง
...
การใช้ Shared Axis ทำให้หลาย Axes ใช้ Scale และ Tick ที่สัมพันธ์กัน จึงเปรียบเทียบตำแหน่งตามเวลาได้ง่ายขึ้น
...
ตัวอย่างนี้ใช้ `sharex=True` เพราะทุกกราฟอ้างถึงกะเวลาเดียวกัน และใช้ `sharey=True` เพราะทุกกราฟมีหน่วย Output Rate เหมือนกัน การใช้ช่วง Y เดียวกันช่วยให้เปรียบเทียบความต่างได้โดยไม่ถูก Auto Scale ของแต่ละ Axes บิดเบือน

ไม่ควร Share Y Axis ระหว่างอุณหภูมิหน่วย °C กับแรงสั่นสะเทือนหน่วย mm/s เพราะความหมายและ Scale ไม่เหมือนกัน

ค่าที่ใช้ได้กับ `sharex` และ `sharey` ได้แก่ `False`, `True`, `"all"`, `"row"` และ `"col"` เมื่อใช้การแชร์แกน Matplotlib จะซ่อน Tick Label ด้านในบางส่วนโดยอัตโนมัติเพื่อลดความซ้ำซ้อน
...
ในโค้ดแบบ Object-oriented API ซึ่งสร้าง Figure ก่อนแล้วเรียก Method ผ่านตัวแปร `fig` ควรเขียนให้ชัดเจนดังนี้:
...
กราฟทุกช่องไม่จำเป็นต้องกว้างเท่ากัน ตัวอย่างเช่น กราฟแนวโน้มตามเวลาควรได้พื้นที่มากกว่า Summary ขนาดเล็ก
...
`fig.tight_layout()` ยังใช้งานได้และเหมาะกับ Layout ทั่วไป แต่ Constrained Layout รองรับกรณีซับซ้อน เช่น Axes ที่ครอบคลุมหลายช่องและ Colorbar ที่อ้างอิงหลาย Axes ได้ยืดหยุ่นกว่า
...
![แผนภาพอธิบาย left, bottom, width และ height ของ fig.add_axes()](./add-axes-coordinate-guide.png)
...
## 10. `GridSpec` สำหรับ Layout ที่ครอบคลุมหลายช่อง

`GridSpec` สร้างโครงตาราง แต่ยังไม่วาดกราฟ จากนั้นจึงนำตำแหน่งจาก Grid ไปสร้าง Axes ผ่าน `fig.add_subplot()`
...
ตำแหน่งแต่ละรายการหมายถึง:

- `grid[0, :2]` หมายถึงแถวบนและคอลัมน์ตั้งแต่ต้นจนถึงก่อนคอลัมน์ที่ 2 จึงครอบคลุมสองช่องแรก
...
เมื่อ Layout เริ่มซับซ้อน ชื่ออย่าง `output_ax` อ่านง่ายกว่า `axes[0, 1]` ฟังก์ชัน `subplot_mosaic()` ให้กำหนดแผนผังด้วยชื่อ และคืนกลุ่ม Axes เป็น Dictionary
...
ชื่อ `"output"` ปรากฏสองช่องติดกันในแถวบน จึงกลายเป็น Axes เดียวที่ครอบคลุมสองคอลัมน์ ส่วน `"quality"` ปรากฏต่อกันในคอลัมน์ขวา จึงครอบคลุมสองแถว

หากใช้ `"."` ใน Layout Matplotlib จะปล่อยตำแหน่งนั้นว่าง Mosaic เหมาะกับ Dashboard ที่ต้องอ่านและแก้ Layout บ่อย เพราะชื่อพื้นที่บอกหน้าที่ของกราฟโดยตรง

---

## 12. ใส่ชื่อรวมและป้ายกำกับแต่ละกราฟย่อย

`ax.set_title()` ตั้งชื่อเฉพาะ Axes ส่วน `fig.suptitle()` ตั้งชื่อรวมของ Figure

เมื่อต้องอ้างถึงกราฟย่อยหรือ Panel ระหว่างอธิบาย สามารถเพิ่มตัวอักษร A, B, C และ D ได้:
...
`transform=ax.transAxes` กำหนดให้ `(0.02, 0.95)` เป็นตำแหน่งภายในกรอบ Axes โดยค่า X เท่ากับ 0.02 คือห่างจากขอบซ้าย 2% และค่า Y เท่ากับ 0.95 คือสูงจากขอบล่าง 95% ตำแหน่งนี้ไม่ขึ้นกับค่าข้อมูลบนแกน ตัวอักษรจึงอยู่มุมเดิมแม้แต่ละกราฟมีช่วงข้อมูลต่างกัน

---

## 13. ปิด Axes ที่ไม่ได้ใช้

บางครั้งจำนวนกราฟไม่พอดีกับกริด เช่น มีข้อมูลห้าชุดแต่สร้าง Layout 2×3 สามารถซ่อนช่องสุดท้ายได้:
...
`axes.flat[-1]` เลือก Axes ตัวสุดท้าย `set_visible(False)` เก็บ Axes นี้ไว้ใน Figure แต่ไม่แสดง ส่วน `delaxes()` นำ Axes ออกจาก Figure การซ่อนช่องที่ไม่ได้ใช้ช่วยให้ Layout ดูสมบูรณ์และไม่เกิดช่องว่างที่ดูเหมือนลืมวาดกราฟ
...
| ให้บางกราฟครอบคลุมหลายแถวหรือคอลัมน์ | `fig.add_gridspec()` |
...
- ใช้ Summary, Filter หรือแยก Dashboard ตามงานของผู้ปฏิบัติงาน
...
หากต้องซูมเพื่ออ่านแทบทุกช่อง แสดงว่า Layout อาจแน่นเกินไป ควรแบ่ง Figure หรือเลือกเฉพาะ Metric ที่ตอบคำถามเดียวกัน
...
- Layout ที่กราฟหลักมีพื้นที่มากกว่ากราฟสรุป
