...
Legend ที่ดีจึงไม่ควรใช้แทนข้อความทุกอย่างในกราฟ และควรมีเฉพาะรายการที่ช่วยแยกความหมายของข้อมูล
...
Matplotlib จะสร้าง Legend จาก Plot Artist ที่มี `label` เมื่อเรียก `ax.legend()`
...
Matplotlib จะละเว้น Artist ที่ไม่มี Label หรือมี Label ขึ้นต้นด้วย `_` ขณะสร้าง Legend อัตโนมัติ คุณสมบัตินี้เหมาะกับเส้นช่วยที่ไม่ต้องการอธิบายในกล่อง Legend
...
- `loc` เลือกมุมของ Legend ที่จะยึดกับหมุดนี้
...
บางครั้งกราฟมีหลาย Artist แต่ไม่ต้องการแสดงทั้งหมดใน Legend หรือต้องการวางสถานะสำคัญไว้ก่อน สามารถรับ Handles และ Labels แล้วเลือกและจัดลำดับใหม่ได้
...
ข้อมูล Smart Farm อาจมีความหมายที่ไม่ได้มาจากเส้นข้อมูลโดยตรง เช่น พื้นที่สีเขียวคือช่วงอุณหภูมิเป้าหมาย และเส้นประสีส้มคือเกณฑ์เตือน สามารถสร้าง Handle ตัวอย่างด้วย `Patch` และ `Line2D`
...
ค่า `s` ของ `scatter()` คือพื้นที่ Marker ในหน่วย point² ไม่ใช่ตารางเมตรโดยตรง จึงใช้ `size_scale` แปลงค่าทางธุรกิจให้เป็นขนาดที่มองเห็นได้
...
อีกทางเลือกหนึ่งคือ `PathCollection.legend_elements()` ซึ่งช่วยสร้าง Handles จาก Scatter อัตโนมัติ แต่ Custom Handle เหมาะกับบทนี้เพราะควบคุมค่าตัวอย่างและข้อความให้ตรงกับงานจริงได้
...
ไม่ควรสร้าง Legend หลายสิบรายการเพื่อแทนความชื้นทุกค่า เพราะ Scale ต่อเนื่องของ Colorbar ใช้เทียบสีกับค่าความชื้นได้โดยตรง
...
- ใช้ชื่อสถานะที่ผู้ปฏิบัติงานเข้าใจตรงกัน
