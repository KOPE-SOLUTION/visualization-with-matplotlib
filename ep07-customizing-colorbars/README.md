...
Colorbar เป็นมาตราส่วนอ้างอิงที่จับคู่สีกับค่าตัวเลข เช่น สีของแต่ละช่องใน Heatmap แทนค่า PM2.5 เท่าไร หรือพื้นที่สีแดงมีอุณหภูมิสูงกว่าค่าเป้าหมายกี่องศา หากไม่มี Colorbar จะทราบเพียงว่าสีแตกต่างกัน แต่ไม่ทราบขนาดและหน่วยของข้อมูล

บทนี้ใช้ **ระบบติดตามคุณภาพอากาศภายใน Smart Building** เป็นกรณีศึกษา โดยจำลองข้อมูลจาก Sensor หลายชั้นของอาคาร แล้วอธิบายการออกแบบ Colorbar ให้สื่อความหมายถูกต้องและเปรียบเทียบข้อมูลข้ามกราฟได้
...
| Mappable | Plot Object ที่เชื่อมข้อมูลเข้ากับ Colormap และ Normalization เช่น ผลลัพธ์จาก `imshow()` หรือ `scatter()` |
...
| Masked value | ค่าที่ตั้งใจไม่แสดง เช่น ข้อมูลหายหรือ Sensor Offline |
...
- `rng` ใช้ Seed คงที่เพื่อให้การรันแต่ละครั้งได้ผลลัพธ์เดิม
...
จุดที่ควรสังเกตคือโค้ดเก็บผลลัพธ์ของ `ax.imshow()` ไว้ในตัวแปร `image` แล้วส่งตัวแปรนี้ให้ `fig.colorbar()`

### Mappable คืออะไร

`image` เป็น Mappable หรือ Plot Object ที่ `imshow()` คืนมา โดยเชื่อมข้อมูลสำคัญสามส่วนไว้ด้วยกัน:

1. ข้อมูลตัวเลขที่นำไปแสดง
2. กฎ Normalization เช่น ช่วงสี 0–30
3. Colormap ที่ใช้เปลี่ยนตำแหน่งบน Scale ให้เป็นสี

เมื่อส่ง `image` ให้ `fig.colorbar()` Colorbar จะใช้กฎ Normalization และ Colormap ชุดเดียวกับ Heatmap สีและช่วงค่าของทั้งสองส่วนจึงตรงกัน

ลำดับการแปลงสีโดยย่อคือ:

~~~text
ค่าจริงจาก Sensor
→ Normalization คำนวณตำแหน่งภายในช่วงสี
→ Colormap เปลี่ยนตำแหน่งนั้นเป็นสี RGBA
→ Plot แสดงสีบนกราฟ
→ Colorbar แสดงมาตราส่วนของสีชุดเดียวกัน
~~~

ตัวอย่าง เมื่อกำหนด `vmin=0` และ `vmax=30` ค่า 15 จะอยู่กึ่งกลางของ Scale หรือมีตำแหน่งสีประมาณ 0.5 จากนั้น `viridis` จึงเลือกสีบริเวณกึ่งกลางมาแสดง ค่าใน Array ยังคงเป็น 15 มีเพียงตำแหน่งที่ใช้เลือกสีเท่านั้นที่ถูก Normalize

`fig.colorbar()` สร้าง Colorbar พร้อม Axes ของตัวเองและคืน Colorbar Object กลับมา ตัวแปร `colorbar` จึงใช้ปรับ Label และ Tick ได้ ส่วน `colorbar.ax` ใช้เข้าถึง Axes ภายใน Colorbar เช่น ตอนเปลี่ยนข้อความ Tick ของ Discrete Colorbar
...
การส่ง `ax=ax` ระบุ Axes ที่ใช้เป็นพื้นที่อ้างอิงของ Colorbar และช่วยให้ Matplotlib จัดสรรพื้นที่ได้ถูกต้อง
...
เหมาะกับหมวดหมู่ที่ไม่มีลำดับ เช่น ประเภทห้องหรือชนิด Sensor แต่ไม่เหมาะกับค่าต่อเนื่อง เพราะความต่างของสีอาจทำให้ดูเหมือนข้อมูลแบ่งเป็นกลุ่มทั้งที่จริงเปลี่ยนต่อเนื่อง
...
ใช้ `.copy()` เพื่อไม่ให้แก้ข้อมูลเดิมโดยไม่ตั้งใจ
...
หากค่าเป้าหมายไม่อยู่กึ่งกลางของช่วง เช่น ต้องการแสดงตั้งแต่ −2 ถึง +6 °C `TwoSlopeNorm` ยังสามารถวาง `vcenter=0` ไว้ที่ค่าที่มีความหมายได้
...
หากต้องแสดงตัวเลขขอบเขตด้วย ควรแสดงใน Tooltip ตารางอธิบาย หรือใช้ Tick ที่ขอบแทนชื่อสถานะ โดยไม่ใส่ข้อความบน Colorbar จนแน่นและอ่านยาก

---

## 9. Missing Data และสีพิเศษ

Sensor อาจไม่ส่งข้อมูลเพราะแบตเตอรี่หมด Gateway Offline หรือ Packet สูญหาย ไม่ควรแสดงค่าที่หายเป็นศูนย์ เพราะศูนย์เป็นค่าตัวเลขที่มีความหมายและอาจทำให้เข้าใจว่าอากาศสะอาดมาก
...
- หากเปลี่ยน Scale แบบอัตโนมัติ ต้องแสดงช่วงค่าให้เห็นอย่างชัดเจน
